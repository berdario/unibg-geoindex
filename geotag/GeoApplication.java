 ...
 * @author  giorgio
 */
public class GeoApplication extends javax.swing.JFrame {
    //COSTANTI
    ...
    private String swLanguage = "./src/stopWords/englishSW.txt";
    ...
    private void initComponents() {
        ...
        org.jdesktop.application.ResourceMap resourceMap = org.jdesktop.application.Application.getInstance(geotag.LocationBasedRetrievalApp.class).getContext().getResourceMap(GeoApplication.class);
            ...
            for (int i = 0; i < nameFiles.length; i++){
                if (!nameFiles[i].isDirectory()){
                        ...
                        try {
                            ...
                            alreadyindexing = contInd.control(nameFiles[i].getName());
                            ...
                            if(!alreadyindexing){
                                do{   
                                    ...
                                    //PRIMA ERA COMMENTATO
                                     contInd.indexing(documentContent, nameFiles[i].getName());
                                    ...
                                    for(int ind = 0; ind < finalGeoWordVector.size(); ind++){
                                        GeographicWord gw = geoWordVector.elementAt(ind);                                    
                                        // cerco nell'r-tree
                                        ...
                                        {
                                        	FileReader file=new FileReader(path+"db"+File.separator+(Integer.parseInt(vettoricodici.codici.elementAt(trovati))/1000)+File.separator+vettoricodici.codici.elementAt(trovati));
                                    		...
                                    		{
                                    			data=line.split("�#");
                                    			if(data[0].equalsIgnoreCase(nameFiles[i].getName()))
                                    				trovato=true;
                                    			line = lr.readLine();
                                    		}
                                        	file.close();
                                        ...
                                        if(trovato==false)
                                        	for(int trovati=0;trovati<vettoricodici.codici.size();trovati++)
                                        	{
                                        		FileWriter file=new FileWriter(path+"db"+File.separator+(Integer.parseInt(vettoricodici.codici.elementAt(trovati))/1000)+File.separator+vettoricodici.codici.elementAt(trovati),true);
                                ...
                                errorLabel.setText("Il file " + nameFiles[i].getName() +" è già stato indicizzato");
...
//                    }
                ...
                if(geoLocation.isLocation()){
                    ...
                    vettori vettoricodici=new vettori();
                    int numeroresults=0;
                    vettoricodici=query_rtree.query(path,geoLocation.getmbr_x1()+allarga, geoLocation.getmbr_y1()+allarga,geoLocation.getmbr_x2()-allarga, geoLocation.getmbr_y2()-allarga);
                    
                    for(int trovati=0;trovati<vettoricodici.codici.size();trovati++)
                    {
                      	FileReader fileletto=new FileReader(path+"db"+File.separator+(Integer.parseInt(vettoricodici.codici.elementAt(trovati))/1000)+File.separator+vettoricodici.codici.elementAt(trovati));
                       	//file.write(nameFiles[i].getName()+"�#"+gw.getGeoScore()+"\r\n");
                      	LineNumberReader lr = new LineNumberReader(fileletto);
                		String line = lr.readLine();
...
*/
    ...
    public String openDocument(String documentName, String documentExtension, File fileName) {        
        ...
        if (documentExtension.equalsIgnoreCase("pdf")) {           
            ...
            errorLabel.setText("File format isn't supported");
        }
    ...
    public String openPDFDocument(String documentName) { 
        ...
        try {
            ...
            errorLabel.setText("Error onening PDF file");
        } catch (CryptographyException ex) {
            //statusMessageLabel.setText("Error");
        } catch (InvalidPasswordException ex) {
            errorLabel.setText("Error: password invalid");
        }
    ...
    public String openHTMLDocument(String docURL){                 	
        ...
        try {
            ...
            errorLabel.setText("Error opening HTML file");
        }
    ...
    public String openXMLDocument(File fileName){         
        ...
        try {              
            ...
            errorLabel.setText("Error parsing XML file");
        } catch (IOException ex) {
            errorLabel.setText("Error opening XML file");
        }
    ...
    static Vector<GeoRefDoc> search(String keyWords, String location){
    	/*
    	 * per ora reso la funzione statica, adatto quindi gli oggetti/variabili
    	 * TODO crea una cmdlinegeoapplication che mantiene lo stato, permettendo l'uso interattivo
    	 */
	    Vector<GeoRefDoc> results = new Vector<GeoRefDoc>();
	    Vector<GeoRefDoc> resultsmerge = new Vector<GeoRefDoc>();
	    String path="./";
    ...
    public static void main(String args[]) {
    	...
    	try {
			...
			//System.out.println(cmd.hasOption("-h"));
			args=cmd.getArgs();
			
			if (cmd.hasOption("help")){
				HelpFormatter formatter = new HelpFormatter();
				formatter.printHelp( "LocationBasedRetrieval", options );
			}
			else if (cmd.hasOption("gui")){
	            java.awt.EventQueue.invokeLater(new Runnable() {
	            public void run() {
	                new GeoApplication().setVisible(true);
	            }
	            });
	        }
